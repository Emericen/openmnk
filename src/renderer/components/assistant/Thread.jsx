...
import { UIPhase } from "@/store/chatRuntimeStore.js"
...
const SystemStatusTextPart = ({ text, isInterrupt }) => {
...
function QueryBar() {
  ...
  const aui = useAui()
  const lastPhaseRef = useRef(UIPhase.READY)
  ...
  const formatInsertText = useCallback((before, after, text) => {
  ...
  useEffect(() => {
    const unsubscribe = window.api.on("dictation", (payload) => {
  ...
  const renderHighlightedTitle = useCallback((title, query) => {
