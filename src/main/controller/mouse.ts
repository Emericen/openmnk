...
export function createMouseController({
  ...
  async function moveSmoothTo(
    ...
    const current = (await mouse.getPosition()) as unknown as Point
  ...
  async function previewPoint(point: Point): Promise<void> {
    await wait(Math.max(20, Math.floor(mouseDelay / 4)))
    await moveSmoothTo(point)
    await wait(Math.max(20, Math.floor(mouseDelay / 4)))
  }

  async function previewDrag(start: Point, end: Point): Promise<void> {
    await wait(Math.max(20, Math.floor(mouseDelay / 4)))
  ...
  async function leftClick(point: Point): Promise<void> {
    await wait(mouseDelay)
    await mouse.move(point as never)
    await wait(mouseDelay)
    await mouse.leftClick()
    await wait(mouseDelay)
  }

  async function rightClick(point: Point): Promise<void> {
    await wait(mouseDelay)
    await mouse.move(point as never)
    await wait(mouseDelay)
  ...
  async function doubleClick(point: Point): Promise<void> {
    await wait(mouseDelay)
    await mouse.move(point as never)
    await wait(mouseDelay)
  ...
  async function leftClickDrag(start: Point, end: Point): Promise<void> {
    await wait(mouseDelay)
    await mouse.move(start as never)
    await wait(mouseDelay)
    await mouse.pressButton(0)
    await wait(mouseDelay)
    await mouse.move(end as never)
    await wait(mouseDelay)
    await mouse.releaseButton(0)
  }

  async function scroll(point: Point, steps: number): Promise<void> {
    await wait(mouseDelay)
    await mouse.move(point as never)
    await wait(mouseDelay)
  ...
  async function nudgeMouse(): Promise<void> {
    const { x, y } = (await mouse.getPosition()) as unknown as Point
    await mouse.move({ x: x + 1, y } as never)
    await mouse.move({ x: x - 1, y } as never)
  }
