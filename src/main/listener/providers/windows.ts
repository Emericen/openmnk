...
type ProviderInputPayload =
  ...
  | { type: "scroll"; rawEvent: WindowsInputEvent; platform?: string }
...
type SubscribableEmitter = {
  on: (eventName: string, handler: (event: WindowsInputEvent) => void) => void
  removeListener?: (
    eventName: string,
    handler: (event: WindowsInputEvent) => void
  ) => void
}
...
function modifierFlagMatches(key: string, event: WindowsInputEvent): boolean {
...
export function resolveWindowsKey(event: WindowsInputEvent): string {
  const rawcode = Number(event?.rawcode)
  if (WINDOWS_RAW_CODES[rawcode as keyof typeof WINDOWS_RAW_CODES]) {
    return WINDOWS_RAW_CODES[rawcode as keyof typeof WINDOWS_RAW_CODES]
  }

  const keycode = Number(event?.keycode)
  const key = WINDOWS_KEY_CODES[keycode as keyof typeof WINDOWS_KEY_CODES]
...
export async function createWindowsIohookProvider({
  onInput,
}: ProviderOptions = {}) {
